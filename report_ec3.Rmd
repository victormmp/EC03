...
O objetivo deste estudo de caso é investigar como modificações de hiperparâmetros de um algoritmo de otimização baseado em evolução diferencial influenciam em seu desempenho, em diferentes cenários de execução. Os algoritmose serão testados  partir de funções de Rosenbrock, de dimensões entre 2 e 150. 
...
Como, a princípio, não podemos assumir normalidade dos dados coletados, para saber a quantidade de instâncias a ser utilizada, fizemos o cálculo considerando o uso do teste de Wilcoxon [@Wilcox], utilizando a função `calc_instances` [@Campelo].
...
Compilando o teste de hipótese de Wilcoxon obtemos os seguintes resultados:
...
Para uma melhoria de resultados, propõe-se o uso da herística citada em [@FCampelo_artigo]. Porém esse método exigiria uma custo computacional muito alto, mostrando-se inviável para o equipamento utilizado para geração dos dados analisados no presente relátorio. 
